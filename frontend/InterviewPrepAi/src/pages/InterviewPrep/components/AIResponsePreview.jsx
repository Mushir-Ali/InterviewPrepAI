        ...
        <div className='text-[14px] prose prose-slate dark:prose-invert max-w-none'>
            ...
            components={{
                code({ node, className, children, ...props }) {
                    ...
                    return isInline ? (
                        <code className="px-1 py-0.5 bg-gray-100 ronuded text-sm" {...props}>
                            {children}
                        </code>
                    ) : (
                        <CodeBlock
                ...
                table({children}){
                    return (<div className='overflow-x-auto my-4'>
                        <table className='min-w-full divide-y divide-gra-300 border border-gray-200'>{children}</table>
                        </div>)
                },
                thead({children}){
                ...
                th({children}){
                    return <th className='px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wide'>{children}</th>
                },
                td({children}){
